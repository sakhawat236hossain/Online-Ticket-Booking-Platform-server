...
async function run() {
  try {
    ...
    // ==================== USER OVERVIEW API =========================
app.get("/user-overview/:email", verifyFBToken, async (req, res) => {
    try {
        const email = req.params.email;
    ...
    app.patch("/makeFraud/:id", async (req, res) => {
      ...
      if (!userToMarkFraud || userToMarkFraud.role !== "vendor") {
        return res
          .status(404)
          .send({ message: "User not found or is not a vendor." });
      }
    ...
    app.get("/admin-overview", verifyFBToken, async (req, res) => {
      try {
        ...
        const totalBookings = await ticketsBookingCollection.countDocuments({
          status: "paid",
        });
    ...
    app.get("/tickets/:id", async (req, res) => {
      ...
      try {
        ...
        if (!ticket) {
          return res
            .status(404)
            .send({ message: "Ticket not found or is currently unavailable." });
        }
    ...
    app.get("/vendor-overview/:email", verifyFBToken, async (req, res) => {
      ...
      const soldBookings = await ticketsBookingCollection
        .find({
          "vendor.VendorEmail": email,
          status: "paid",
        })
